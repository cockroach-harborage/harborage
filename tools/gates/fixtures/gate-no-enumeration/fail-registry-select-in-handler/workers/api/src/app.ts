...
app.post('/api/help/offer', async (c) => {
	// A read-then-write, added to improve an error message. This is the shape the
	// rule exists to refuse: a route that can read the registry is a route that
	// can be asked about a person.
	const existing = await c.env.DB.prepare('SELECT id FROM helper_offers WHERE dedup_token = ?1').first();
	if (existing) return c.json({ ok: false }, 409);
