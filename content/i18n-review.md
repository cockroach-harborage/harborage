...
| `apps/web/messages/{en,hi}.json`: canary + peer-share strings | `canary_*`, `share_*` | ★ safety-critical (the safety-signal state and the "do not trust an unverified copy" warning). EN authored to the plain-language rules; HI DRAFT, needs a named reviewer before the canary is established | UNASSIGNED |
| `apps/web/messages/{en,hi}.json`: live-board strings | `board_*`, `band_*`, `sig_*`, `report_*` (36) | ★ safety-critical (the nine hazard names, the STALE line, and the "not live watching" limit). EN authored to the plain-language rules; HI DRAFT, needs a named reviewer before `live_board` flips. A mistranslated `sig_safe_exit` or `sig_kettling_risk` sends someone the wrong way | UNASSIGNED |
| `apps/web/messages/{en,hi}.json`: accountability strings | `acct_*` (20) | ★ legally sensitive rather than life-safety: "documented allegation under lawful process, not a finding of guilt" has to survive translation, and a HI rendering that reads as an accusation is a defamation exposure. EN authored to the plain-language rules; HI DRAFT, needs a named reviewer AND counsel before `accountability_records` flips | UNASSIGNED |
