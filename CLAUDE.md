...
- **Abuse / DDoS / DoS defense.** Turnstile + layered rate limits + reputation-gating in front of any intake or AI. (**Corrected 2026-07-25:** there is no Cloudflare setting that "tunes Turnstile to admit Tor/VPN" — no such control is documented and the official guidance points the other way. What actually helps those users is widget-side: Managed mode rather than Invisible, so a blocked user has a recovery path, plus pre-clearance. Set `feedback-enabled: false`; it defaults true and reports visitor feedback to Cloudflare.) **Volumetric L7 DDoS relies on Cloudflare's managed/unmetered rulesets;** WAF custom-characteristic rate-limiting is unavailable on the funded plan, so the **RateLimit DO is the deliberate app-layer substitute.** Never gate *reading* public safety info. No raw-IP logging on protestor endpoints. AI moderation runs behind spend caps with a kill-switch that degrades to human-only.
...
- **≥2 reviewers + CODEOWNERS on every sensitive path — CURRENTLY UNMET, and stated plainly rather than pretended.** With a single maintainer this property does not exist and no repo setting creates it: branch protection has `required_pull_request_reviews: null`, and CODEOWNERS names one account, so even enabling reviews would be self-approval. Do not fake it with a second account or a bot approver — that produces the appearance of the control without the substance, which is worse than the gap. Until a second maintainer exists, sensitive-path changes are protected by CI gates, signed commits, and the self-review checklist below only. **This is a named residual risk, not a satisfied standard.** The rule as intended: `workers/**` (esp. auth/crypto/location/evidence), `migrations/**`, `.github/workflows/**`, `infra/**wrangler*`, all signing/canary code, the frozen crypto module, and the DO memory-only invariant classes. A single compromised or coerced contributor/reviewer must not weaken a user-protecting default. (Publication is quorum-required; removal is single-reviewer — an infiltrated reviewer's max unilateral harm is suppression, an accepted fail-safe direction.)
...
- [ ] **No secret in the diff**; actions SHA-pinned; **commit signed**; CSP/Trusted Types intact; CODEOWNERS paths identified (≥2 reviewers is currently unmet — see §5; say so rather than ticking it).
...
- **Autonomous ceiling stays LOW and honestly labeled.** The highest state
  ...
  (sub-amplification) reach. Its public label is the plain "Confirmed by
  people nearby" and nothing else: **no "AI-checked" string ever reaches a
  user** (ARCHITECTURE §18.1, which corrects the earlier wording that used to
  sit here; `gate-ai-tells` fails the build on it). The honest "automated, can
  be wrong, not yet human-verified" wording lives on tap-through only.
  `Human-Verified` and full reach are Layer-B (human) only. Never introduce an
  autonomous path to "Verified" language or top-tier reach. Mapping an internal
  state UP to a stronger public label is a truth failure, not a copy nit: the
  label is the entire basis on which a reader decides whether to act.
