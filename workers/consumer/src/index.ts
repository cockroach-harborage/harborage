...
import { dispositionFor } from './life-safety.ts';
...
interface VerificationStateStub {
	apply(
		...
		actorClass?: 'auto' | 'human'
	): Promise<unknown>;
}

export default {
	async queue(batch: MessageBatch<unknown>, env: ConsumerEnv): Promise<void> {
		// SWITCH ON THE QUEUE FIRST, before anything else. Without this a
		// life-safety message falls into handleBatch, which loads the Tier-0
		// ruleset: a KV read on the life-safety path, for a message that will only
		// ever be logged.
		if (batch.queue === 'life-safety') {
			let acked = 0;
			for (const message of batch.messages) {
				// No D1 write on this lane at all, and nothing from the payload is
				// logged: a table or a log line saying when life-safety flows were
				// failing is a durable record of when a crackdown was happening.
				if (dispositionFor(message.body) === 'ack') {
					message.ack();
					acked++;
				} else {
					// Retry rather than ack, so a malformed message reaches the DLQ where
					// an operator can see it. A silent ack is the one outcome with no
					// trace at all.
					message.retry();
				}
			}
			safeLog('queue_batch', {
				...
				outcome: acked === batch.messages.length ? 'all_acked' : 'some_retried'
			});
			return;
		}

		const outcomes = await handleBatch(batch as unknown as Parameters<typeof handleBatch>[0], {
			...
			async recordKeyring(ring: RecordedKeyring): Promise<void> {
				...
				await env.DB.prepare(
					...
					.bind(ring.originalSha256, ring.tier, ring.keyring, ring.copyCount, ring.createdBucket)
					.run();
			},
