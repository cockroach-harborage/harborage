...
- **Structured, ACLED-style taxonomy** (typed: type, date, time-window, location at a controlled granularity, actor by *role/unit/official-identifier*, reported injuries/detentions, constrained narrative). Structured data is dedupable, analyzable, and moderatable; free-text-only is not.
  - **Frozen closed `type` set** (Session 2.7; two-person-reviewed, EN+HI; not limited to six — the widest honest coverage): `baton_charge` (baton charge / beating), `crowd_weapons` (tear gas / crowd-control weapons), `firing` (firing / live ammunition), `assault` (assault / physical violence), `sexual_violence`, `detention` (detention / arrest), `custody_abuse` (abuse or torture in custody), `death` (in custody or protest), `denied_care` (denied medical or legal help), `incommunicado` (held incommunicado / disappearance), `surveillance`, `network_shutdown` (network / communications shutdown), `property_damage` (property damage / demolition), `intimidation` (threats or intimidation). Severe categories still obey every redaction/PII rule and route individual-identity content to the counsel-gated accountability path (§4.10); the category label itself is public-safe. Ships today as a typed module + message strings (`apps/web/src/lib/incident-types.ts`); the signed `content/taxonomy` pack is the future update channel.
...
| **On-device media pipeline, fixed order that can't be bypassed** (ARCH §7.5): strip metadata → cover (solid-fill) + before/after confirm → make a small legible **public copy** → seal the **pristine original**. Runs off the UI thread on a low-end phone; adds no heavy download on open. | The public copy is downscaled only to where a **badge number / banner / official's face stays readable** (never below that floor); redaction happens **on the phone** — the platform only ever gets the sealed (unreadable) original or an already-covered copy |
| **Honest about video (day-1):** the phone does **not** transcode video or cover faces in video; it shares only a **covered still frame** and seals the full clip privately; face-covered video is prepared later, server-side | Copy states plainly that the video's faces are **not yet covered** and the clip stays sealed/private until a safe-to-share version exists — no false promise on a phone that can't deliver it |
...
| **Documented-first low-coverage submit** (2G/flaky/onion): on submit, (1) the incident record + hashes register in seconds — "Documented" fast even on 2G — (2) the small redacted public copy sends next, (3) the big sealed original uploads in the background, resumable, preferring Wi-Fi, across many sessions | Nothing blocks the user; "Saved. Will finish sending when you have signal." The encrypted outbox survives force-quit, is user-viewable/cancellable, and is cleared by panic-wipe; direct-to-R2 (bytes never touch the Worker); dedup of viral reposts skips re-upload without leaking who-has-what (ARCH §7.6) |
...
| Two-sided anti-honeypot: **proof of continued presence before any helper is exposed** (corrected M4, 2026-07-26) | The mechanism is a commitment made at need-creation whose preimage must arrive on a SECOND, separately-ticked request before the responder's card is released. It does NOT verify provenance, and the earlier wording overclaimed: whoever planted a fake need also chose the secret, so producing the preimage costs them one extra round trip. What it does buy is a commitment that cannot be moved to another need, two time-separated and individually rate-limited requests per exposure (so enumeration is linear in wall-clock ticks rather than free), and exposure as a deliberate act rather than an automatic consequence of posting. Real provenance is vetting, and `PINNED_VETTING_ISSUERS` ships empty. Overclaiming here gets a lawyer arrested. |
...
> **See ARCHITECTURE §19 for the low-bandwidth media & upload design.** On very low / flaky coverage: media is optimized on-device (small redacted derivative uploads first, the pristine original never lossy-compressed and syncs in the background, resumable across app kills); the app shows "Saved — will finish sending when you have signal," an honest per-item data cost, and the custody status of each original. The sealed original never auto-pushes over 2G.
...
| **Adaptive media by measured connection** (ARCH §7.5): smaller covered copy on 2G (measured via connection hints + a throughput fallback); the multi-MB sealed original **queues and sends later** on a better/unmetered link — never auto-pushed over 2G | Each item shows its **data cost** and whether it "sends now vs later"; nothing blocks the user waiting on an upload; a running tray shows what's queued to send |
| **Resumable across kills & sessions** (ARCH §7.6): the big sealed original uploads via R2 resumable multipart; completed-part state (uploadId + ETags) persists in the encrypted outbox, so a 30-min 2G upload **survives force-quit and network changes** and continues on next app-open; part URLs re-mint on demand; exponential-backoff+jitter retries; viral-repost dedup skips re-upload | Flushes on `online`/next-open (the only reliable trigger on iOS PWAs — no background execution); pending items are viewable/cancellable; panic-wipe clears the outbox; the existence check is rate-limited and cohort-gated so it never becomes a who-has-what leak |
