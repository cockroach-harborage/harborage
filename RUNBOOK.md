...
7. **Read the `tofu plan` (the human gate).** The `infra (tofu plan)` job runs before the approval gate and prints the plan to the run summary, so it is visible when you are asked to approve. Any change or destroy on Email/DNS/Access/signing-key config is a bug: stop and investigate, never apply. A **Destroy guard** step now fails the build on any destroy or replace — including SPF/DMARC, which `prevent_destroy` does not cover — so this is enforced rather than remembered. A deliberate removal needs the `HB_ALLOW_DESTROY` repository variable set to the exact addresses, recorded here.
