...
7. **Read the `tofu plan` (the human gate).** The `infra (tofu plan)` job runs first and prints the plan to the run summary, so it is visible when you are asked to approve the **apply** (the first approval only lets the read-only plan run). Any change or destroy on Email/DNS/Access/signing-key config is a bug: stop and investigate, never apply. A **Destroy guard** step now fails the build on any destroy or replace — including SPF/DMARC, which `prevent_destroy` does not cover — so this is enforced rather than remembered. A deliberate removal needs the `HB_ALLOW_DESTROY` repository variable set to the exact addresses, recorded here.
...
Per-feature kill switches and heightened-threat mode are **runtime data** flipped instantly from the Access-gated `/console` (backed by the FlagState DO + `FLAGS` KV; fail-closed by default). They are **not** a deploy and **not** a manual infra step — during a live crackdown you flip a flag from the console, you do not push code. **Be honest about the latency: propagation is up to ~2 minutes**, not seconds (60 s KV cache TTL + up to ~60 s KV global propagation). Plan the incident response around two minutes. Irreversible / accountability-publication / detainee-intake toggles require 2-person authorization.
...
| Org-level Access Independent MFA + AAGUID list | AAGUID pinning needs real ceremony hardware. **Correction (2026-07-25): the "cannot be tofu" half is no longer true** — Cloudflare provider 5.22 exposes `cloudflare_zero_trust_organization.mfa_config` and `cloudflare_zero_trust_list` with `type = "AAGUID"`. It stays manual for now only because that resource has **no `terraform import`**: adopting it means Tofu authoritatively writing org-wide settings it never created, on the resource gating the only privileged surface. Move it to IaC in a dedicated PR with `prevent_destroy` and every existing field mirrored, never as a drive-by. |
...
| Read the `tofu plan` before apply | The human gate against a destructive/compelled change (now backed by the automated destroy guard, which fails closed) |
| Production deploy approval | **Corrected: this is a ONE-person gate today.** `bootstrap.sh` sets the required reviewer to the account running it, i.e. the same person who pushes, so it is a confirmation dialog rather than two-person authorization. It is still worth keeping (it forces a deliberate pause and a look at the diff), but do not describe it as two-person until a second reviewer exists on the `production` environment. |
| **Step-by-step for all of these: [docs/maintainer-walkthrough.md](./docs/maintainer-walkthrough.md)** | Written for the current solo-maintainer reality, with the multi-person steps explicitly parked until people join after the build. |
| Generate the security-contact age key | Identity-optional encrypted disclosure intake is promised in CLAUDE.md and does **not** exist; `/.well-known/security.txt` currently admits the gap. Generate an age keypair offline, publish the public half, add the `Encryption:` field. Until then a researcher with a de-anonymization finding must identify themselves to GitHub. |
| Register the commit-signing key on GitHub | Commits are signed locally but the key is not registered, so every commit shows "unverified" and `required_signatures` stays off — while CLAUDE.md lists signed commits as binding. One command: `gh ssh-key add <key>.pub --type signing`, then re-run `github-setup.sh` with `ENFORCE_SIGNATURES=1`. This is a real property a solo maintainer *can* have. |
