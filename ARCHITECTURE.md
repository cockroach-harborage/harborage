...
| 5 | Live Ground Situation | One LiveBoard DO per zone + **HLL** exact lower bound; memory-only; long-poll + client heartbeat replay | Red line 3 params (§6). Sharding deferred (shared-salt blocker, §6.2); hibernation dropped (eviction/delay overlap). |
...
- **`web`** — SvelteKit SSR + Static Assets. **Config note (M1):** with `@sveltejs/adapter-cloudflare` the generated `.svelte-kit/cloudflare/_worker.js` is the `main` entry and owns SPA-fallback + 404 routing, so `not_found_handling` / `run_worker_first` are **not** set manually in `apps/web/wrangler.jsonc` (the adapter handles it; the 404 path is e2e-verified). The intent below — serve assets without spending Worker CPU — holds via the adapter's own asset routing. 128 MB isolate is a hard ceiling — SSR stays tiny; **no image/video decode, no perceptual hashing here** (impossible in the Worker isolate). Route that work off-isolate: the **Images binding (`env.IMAGES`)** for resize/normalize/`.info()`, and **Cloudflare Containers** (up to 4 vCPU / 12 GiB, ffmpeg/OpenCV/pHash) for heavy image or any video work (see §14).
...
- **Queue consumer** — Tier-0 lexical/hash triage, notification fan-out, corroboration bookkeeping. **DLQ mandatory** or poison messages are silently discarded. Messages ≤128 KB (put blobs in R2, pass the key). Drain latency is real: sustained surge can make queued items minutes-to-hours late → a **dedicated high-priority intake queue for medical/detention** so life-safety items aren't stuck behind bulk feed moderation. **Corrected 2026-07-25: Cloudflare Queues has no "reserved concurrency".** `max_concurrency` is a per-consumer *ceiling* (1-250), not a reservation, and there is no priority mechanism between queues. The life-safety guarantee therefore rests on queue separation plus `metrics()`-driven operator paging, not on a reservation that does not exist.
...
- `legal_matter_refs` — `{ref_hash, kind, next_deadline_hour, fired, shard, created_hour}` (migration 0025). *No* party-name column, and **narrower than this section previously specified** — corrected at build (M5, 2026-07-26). Three fields were dropped rather than built. `next_deadline_ts` became `next_deadline_hour`: an Article 22 production deadline stored to the second **is an arrest timestamp**, because subtracting 24 hours yields the minute a named person was taken, for every row, and PITR keeps it ~30 days regardless. `public_case_number` is gone even though case numbers are public record — a public case number is the strongest available **join key back to a named person**, so holding one turns an opaque ref into an identity lookup. `jurisdiction_bucket` is gone because a locality joined to a deadline hour is a detention-intensity signal tied to a place. `status` is gone: `fired` is the only state the platform needs, and anything richer describes the matter.
- `notices` + `notice_chain` — signed notices + append-only hash chain. Migrations `migrations/0006_notices.sql`. The signed payload is the canonical-JSON of `{id, epoch, notice_type, title_i18n, body_i18n, area?, published_at, supersedes?}` (sorted keys, compact — the same canonicalization the content packs use); `payload_hash = SHA-256(canonical)`. Each of m independent role keys signs the 32-byte `payload_hash`; the notice type sets m (below). The chain is `H_0 = 32 zero bytes`, `H_i = SHA-256(H_{i-1} ‖ payload_hash_i)`; the `NoticeLog` DO computes + stores Merkle checkpoint roots every 64 entries or daily, and **signing a checkpoint stays an offline manual act** (like the canary). Directive-type→required-m (counsel-gated, tunable upward only): `safety_directive`/`detention_alert` = 3; `logistics`/`legal_status`/`correction`/`transparency` = 2. Because `key_directory` ships empty until the ceremony, every verify fails closed.
- `key_directory` / `revocation_list` — signed, versioned, epoch-stamped. Migrations `migrations/0007_key_directory.sql`.
- `reputation_scalars` — `{key_hash, compartment, scalar_milli, coarse counters, epoch}`, per-compartment, no edges. **Milestone, settled 2026-07-25: table + code at M2, WRITES flag-gated, switch-on at M3.** The earlier note put the whole thing at M3 because creating the table early would accumulate a compellable per-compartment roster over the D1 Time-Travel window while gating nothing. That objection is about **rows, not tables**: with every write behind the fail-closed `community_corroborate` flag the table holds zero rows until switch-on, which is exactly the build-now-flip-later split §18.2 prescribes. Switch-on remains additionally gated on blind-token-carried reputation so the server holds no list at all. Migration `migrations/0009_reputation_scalars.sql`.
- `corroborations` — `{entity_id, dedup_token, stance, at_bucket, epoch}`, one row per (item, corroborator), recording THAT a distinct participant acted and never who. `dedup_token = HMAC(per-item salt, key hash)`. **Honest limit:** while the salt exists this is a membership oracle over one item's corroborators — anyone holding the salt can test a candidate key. Minimized by a per-item random salt destroyed when the corroboration window closes, swept rows, no reverse index and no join to `reputation_scalars`; the residual is that PITR keeps the salt ~30 days regardless. The oracle-free construction needs blind tokens and is M3. Migration `migrations/0011_corroborations.sql`.
- `mod_audit` — **the single canonical definition; §7.4 and the §15 purge table previously gave two conflicting column sets and now point here.** Union of both, since each carried a field the other needed: `{opaque_id, category_code, action, actor_class, reviewer_ref?, model_version?, confidence_milli?, prior_state?, new_state?, reason_code?, at_bucket}`. `reviewer_ref` is NULL for autonomous Layer-A rows and set for human ones; `confidence_milli` carries the ≥0.95 threshold rule; `at_bucket` is a coarse day bucket, never a precise action timestamp. **Non-content by construction** — never the payload, no author, no reporter, no IP, no geo. Appeals run off this row. Custody: ENCRYPTED-AT-REST per §4.1, which this document rates "**No** — treat as compellable"; D1 Time Travel is ~30 days and cannot be disabled, so nothing here may be described as ephemeral.
- **Action vocabulary is fixed and contains no purge verb.** `{label, rank, hide-pending, retain-pending, route-to-gate}` (§15). Where §15's radioactive table reads "short-purge", that is a **Layer-B human capability, separately gated** — there is no autonomous purge path in code, and the conformance test asserts no route from a model or community output to publish/delete/unredact/name. Autonomous radioactive handling is hide-pending + retain-pending + route-to-gate.
...
**v1 D1 is a single unsharded database per logical DB** with disciplined indexing (`tools/gates/gate-d1-index.mjs` fails CI when a Worker/app query filters on a column that is not indexed — conservative, query-driven). At launch volumes a well-indexed D1 is nowhere near the 10 GB cap; sharding is a pre-authored, metrics-triggered scale-up, not a launch task. Read-heavy public boards use **Cron-materialized rollup/aggregate tables** (not per-request scans) everywhere — the pattern Domain H already uses, applied to feed/incident public views too.
...
- **Compartments are a closed, append-only enum** (`packages/crypto/src/compartments.ts`): `document(0) directory(1) community(2) accountability(3) curation(4) medical(5) aid(6) legal(7)`. The ordinal is a wire value (cap-cert byte 3), so the list is never reordered and an ordinal is never reused; only `document` and `directory` are active at M2 and the rest are reserved names so the frozen module does not have to grow. The **epoch is device-local and monotonic** — never a server value, because a server that could move it could force rotation on demand and watch which pseudonym disappears — and is not on the wire, since the public key is the identity.
- **Every signature carries a domain-separation context** (§17.6). One compartment key signs several protocols, so `sign()`/`verify()` take a mandatory `SigContext` and frame it `u8(len) ‖ utf8(context) ‖ message`; the length byte makes the tag set prefix-free. `gate-sig-context` keeps raw curve `.sign()` out of reach outside the frozen module.
- Keys cached as **non-extractable `CryptoKey`** in IndexedDB (re-derivable from mnemonic). **The root is held as a non-extractable WebCrypto `HKDF` key, not as bytes** — WebCrypto HKDF does Extract-then-Expand in one call, so `deriveBits(salt="harborage/v1", info="compartment/<d>/<e>")` is byte-identical to the noble path, and after creation nothing can read the seed back out while per-request derivation still works. Conformance-tested in both directions (`packages/crypto/test/device-keys.test.ts`), including that WebCrypto and noble Ed25519 signatures are byte-equal, so a tier switch is invisible. Fallback ladder: WebCrypto secure-curve → WebCrypto P-256 → `@noble` in-memory (reduced-trust, **read-only public path only**, surfaced in plain copy rather than degraded silently). *Implementation note that cost real debugging time:* verifying a WebCrypto ECDSA signature with `@noble/curves` needs **both** `prehash: false` (noble hashes for you by default) **and** `lowS: false` (WebCrypto does not normalise S, so the default rejects about half of genuine signatures). Left at the defaults it fails intermittently, only on the phones that need the fallback tier.
...
BIP39 re-entry re-derives the whole tree. No server, no PII, no email/SMS reset — its **absence is the point** (no compellable backdoor). "Lose it = lose account," stated verbatim in-product.

**Backup-words lifecycle (maintainer decision, 2026-07-25).** An identity is created silently at first send (PRD §15 flow B) or explicitly from `/settings/identity`. Backup then sits `pending`: the words are held on the device so they can be shown, and the user is prompted once to write them down. Confirming — re-typing 3 of the 12 at randomly chosen positions — **erases the words by default**, leaving only non-extractable keys. One setting, **default OFF**, keeps them instead: *"Keep the backup words on this phone."* It exists because some people genuinely cannot write things down, and it trades permanent account loss for device-seizure exposure. Both directions of that trade are stated in the honest-limits copy, and the switch cannot be re-enabled once the words are erased — offering a toggle that silently does nothing would be worse than not offering it. Restore starts `erased`, since the user demonstrably holds the words elsewhere. Optional SLIP-39 Shamir split of the mnemonic for distributed backup; optional BIP39 passphrase (25th word) selects a decoy tree — **best-effort only** in-browser (no hidden-volume guarantee; coercive unlock defeats it; don't oversell until APK).
...
**Incident metadata envelope (M2, §19.1).** The register body is sealed to an **intake keypair** whose private half is a wrangler secret on `workers/consumer` only, and whose public half is served from `GET /api/intake/status`. This is deliberately **not end-to-end**: the incident note is PUBLIC-PLAINTEXT-destined, so the consumer must be able to read it. What it buys is hop confidentiality and blast-radius hardening — the body is opaque in transit, opaque in the queue, and opaque to a compromised edge Worker or an accidental log line. It buys **nothing** against legal compulsion, which is the stated adversary. `tools/gates/sensitive-endpoints.json` classes the endpoint `SEALED-TO-PLATFORM`, and `gate-sealed-body` requires the opening key to be **named and justified** there: every unseal-shaped binding in the codebase must be registered. Registering it honestly is the point — the alternative was picking a binding name that slipped past the pattern, which leaves the check green while the property stops holding. The evidence **original** is a different object with a different claim and stays E2E.

**Custody is scoped to a sealed object, not to a deployment (corrected M3, 2026-07-25).** The M2 form of that gate also refused a `SEALED-E2E` entry if *any* unseal-shaped binding existed anywhere. That is the wrong shape of claim: "the platform holds no key" is a statement about **one body of ciphertext**, and `INTAKE_PRIVATE_KEY` opens the incident metadata envelope while saying nothing about an evidence content key. Left as written, the class became unreachable the moment one legitimate platform key existed, and the only ways forward were to delete the check or to rename the binding until the pattern stopped matching — both of which leave a green tick over an unenforced invariant. Every registry entry therefore names a **`sealed_object`** (required, no default — either default would silently decide a custody question on the author's behalf), a `platform_key` is registered as opening its own entry's sealed object and nothing else, and a `SEALED-E2E` entry fails if a registered key opens **its** sealed object **or** if any unseal-shaped binding is unregistered, since a key whose scope nobody wrote down has to be assumed to open everything. That second rule is what stops the scoping from becoming an escape hatch: the way to pass is still to not hold the key, never to keep quiet about holding it. Proven by `fixtures/gate-sealed-body/fail-e2e-same-lane/` (E2E over a lane a key opens), `fail-no-lane/` (no `sealed_object`) and `fail-key-outside-worker-lib/` (an Env contract in another package, which the previous single-path scan could not see).

The construction is libsodium's `crypto_box_seal` with HKDF in place of the blake2b derivation, on `@noble` and with **no WASM**: §5 confines libsodium to behind the evidence vault, and a 2G phone should not load 300 KB of WASM to file a note. Both public keys are bound into the KDF info, so a captured ciphertext cannot be re-presented as sealed to a different recipient. The client **pins the intake key on first use** rather than having it baked into the build: the app shell is served from the same edge that serves the key (§9.5), so a pinned constant would look stronger while protecting against nothing an adversary who can swap one could not also do to the other. Pinning makes a *later* swap visible, which is the honest, modest claim.
...
- **Tier B (sealed/detainee-linked):** **Critique fix — quorum topology, not just t/n:** no set of holders reachable within one jurisdiction may reach threshold. The **offshore custodian's share is MANDATORY in every quorum** (offshore + one of {reporter, lawyer}). Reconstruction on an **air-gapped offshore device.** This defeats platform compulsion and single-point seizure; it does **not** defeat rubber-hose on a coerced human. Counsel sets the split.

  **Corrected at build (M3, 2026-07-25): plain "Shamir 2-of-3" cannot express the requirement in the sentence next to it, so it is not what ships.** A 2-of-3 split over {reporter, lawyer, offshore} is satisfied by {reporter, lawyer} — both reachable in one jurisdiction, offshore not involved — which is exactly the compulsion this tier exists to defeat. Threshold schemes have no notion of a *mandatory* participant; you get one by construction instead. `packages/crypto/src/vault-key.ts` splits `CEK = K_offshore XOR K_domestic`: the offshore half goes only to the custodian, the domestic half to the domestic holders (sealed directly at t=1, Shamir-split at t≥2, since a threshold of 1 is a degenerate split that hands every holder the whole secret while looking like a threshold scheme). Reconstruction therefore needs the offshore half **plus** a domestic quorum, and compelling every domestic holder yields an independent uniform random value that reveals nothing. `PINNED_CUSTODIAN_KEYS` ships **empty**, so every wrap refuses until a custodian organisation exists — the switch-on gate made structural rather than procedural.
...
Two layers (trimmed to the operable 80%): **(1) CI supply-chain** — Sigstore cosign keyless (GitHub OIDC) + Rekor + SLSA provenance, no long-lived CI key. **(2) User-facing root** — **minisign** detached signatures over release artifacts + offline knowledge packs, signed by the offline m-of-n project key, verified in-app against the pinned pubkey. SW bakes in the pubkey and refuses assets not matching the signed `{path:sha256}` manifest + SRI. **Build status (M1):** the SW signed-manifest verification is **deferred** — it is inert without a signed manifest (the offline key does not exist until the ceremony) and per §9.5 gives broad-tamper *detection*, not targeted-hit *prevention* (the APK is the real fix). The XSS/code-injection hardening that IS effective now — strict nonce/hash CSP + **Trusted Types on every served page** (enforced via `apps/web/_headers` for prerendered pages and the kit.csp meta for script-src, both gate-csp-checked) — ships at M1. *Also deferred to post-v1 stretch (research-grade, won't be operated correctly by volunteers at launch):* client-side gossiped Merkle equivocation detection and bit-for-bit reproducible-JS. Ship a public "verify-what-the-edge-served" watcher as **detection of broad tampering** — never prevention of a targeted hit.
...
- **One LiveBoard DO per zone**, maintaining a **HyperLogLog sketch** (p=12, m=4096) over per-(zone,epoch) HMAC dedup tokens. **Sharding is deferred** — corrected during the M5 build. The blocker is named in `merge()`'s docstring: sharding needs a **shared per-(zone,epoch) salt**, or the same reporter gets a different token per shard and inflates straight past the density floor. `merge()` exists in the pure module so the scale-up is a routing change, and §18.4's `memoryUsageBytesP99` alarm is the trigger.
- The floor consumes a **distribution-free exact lower bound**, not an estimate: every distinct token sets exactly one register non-zero and merge is register-wise max, so `occupied() ≤ n_true` for any hash, any input, any merge — confidence 1, no `z` term. Every deviation is downward, so a register collision withholds publication one tick longer. Bands, which live above saturation, use a **3σ one-sided haircut** (σ = 1.04/√m = 1.625%, `L = Ê × 0.951`), rise on the LCB and fall on the UCB. **No HLL++ bias correction:** its tables move estimates *up*, and up is the unsafe direction.
- **HLL is not here for memory.** 200 reporters in a `Set` is nothing. It is here because a `Set<dedup_token>` **is** the who-was-where structure red line 3 forbids, just in RAM: it answers "was this reporter in this zone" exactly, and it grows unbounded. Honest limit: anyone holding the epoch salt plus a candidate credential can test `reg[i] >= ρ`, a probabilistic membership oracle worth roughly p+ρ bits. What carries the privacy property is that the salt is **memory-only, never logged, never reachable from the Worker, and rotated every 15 minutes** — not the sketch.
- **Salt rotation resets the sketch**, so the density floor would be re-earned every epoch and a publishing zone would strobe dark at every 15-minute boundary. Fixed with **overlapping epochs**: the previous salt and sketch are kept for a 90 s window and combined with `max`, **never `sum`** — summing double-counts a reporter who reported on both sides of a boundary, and inflation is the direction that pushes a small group over the floor. The carry-over applies only across **contiguous** epochs; a gap discards it, because a quiet board must not hold a fresh lease forever.
- **WebSocket Hibernation is dropped, and `gate-memory-only` is not the reason.** Per-socket state is one zone id, which fits in hibernation tags, and tags are not barred. The real problem: an idle non-hibernatable DO is evicted at **70–140 s**, while §6.4 mandates a publication delay of **60–180 s**. Those overlap, so a hazard signal can be destroyed before it is publishable — a life-safety read going dark, forbidden by §6.5. That is a property of memory-only plus a mandated delay and is present with hibernation *off*; anyone who "fixes" it by adding long-lived sockets has fixed nothing. Two lesser reasons: `setWebSocketAutoResponse` is barred by `gate-memory-only`'s `DURABLE_RE`, so every keepalive ping wakes the DO and hibernation's economics evaporate; and the board is never idle during a protest, so hibernation pays exactly when the feature is worthless.
- **M5 ships long-poll only.** The in-flight request and its `setTimeout` are what keep the instance resident through the delay window, so the puller's open request is the clock *and* the keep-alive. A parked reader carries its own **posture** (ordinary or heightened-threat) and is woken under it; one composed view handed to every waiter would deliver the crowd band to a heightened-threat reader at exactly the moment the mode exists to withhold it.
- **The replay source is the reporters' phones, not the server.** The earlier "short rolling replay" was wrong: a memory-only DO has none. Clients re-post every **45 s ± 10 s while the tab is visible**, up to the 10-minute TTL. 45 s sits under the 70 s eviction floor, and the heartbeat is **idempotent for free** — same certificate, same epoch, same token, so the HLL insert is a no-op. The heartbeat is simultaneously the durability mechanism, the clock and the keep-alive; when nobody reports there is no clock and nothing needs one. Honest failure modes, and they belong in the copy: heartbeats stop when the app is backgrounded, so a signal lapses about a minute after the last reporter pockets their phone — **this board works while people are looking at it; it is not live monitoring**. If every reporter drops for more than 70 s the delay clock restarts *later*, never earlier. After eviction the board returns `rebuilding: true` for the first few ticks while the **client keeps showing its cached rows under a STALE badge**, because flashing to empty is indistinguishable from "no hazards here". Rejected: letting the client assert `first_reported_at`, which hands the client control of the publication delay — precisely the attack the delay defends against.
- **The publication jitter is HMAC-derived per (zone, signal, epoch), never per read.** A per-read roll would let a client polling twice a second watch the signal blink and pin the true report time to ±1 poll, making the jitter theatre.

### 6.3 Ingest & dedup

Ingest Worker validates against a schema with **no lat/lng field** (enumerated fixed-grid zone id only); rejects any coordinate-bearing payload; scrubs `CF-Connecting-IP`/geo before emit. `dedup_token = HMAC(in-memory per-(zone,epoch) salt, certHashHex)`; salt never persisted, rotates ~15 min, zone folded in → tokens unlinkable across epochs and zones. **Corrected during the M5 build:** the earlier `reporter_session` let a client mint unlimited apparent reporters with a fresh random seed per heartbeat, defeating the density floor and the corroboration bar entirely. `certHashHex` is a value `verifyRequestCredential` already computes, and the token is derived **inside** the DO — the salt is never returned to the Worker, because a salt the Worker can read is a salt a compelled Worker can be asked for. The route therefore requires the **long-lived** credential and refuses a one-shot: a fresh certificate per heartbeat is a fresh apparent reporter every 45 seconds. **Honest ceiling:** one Turnstile-passing cap-cert is one reporter, so the density floor is exactly as strong as personhood-lite plus the rate ladder and no stronger. It protects a small group from a casual observer, not from a resourced adversary who mints five credentials. **Marshal quorum:** SAFE_EXIT / DISPERSAL publish only with **≥2 valid role-bound marshal Ed25519 signatures** (in-directory, not-revoked); community versions without quorum are withheld, not shown low-confidence.
...
- ~~Hindi/other-language safety-critical verdicts are **advisory only → human review**.~~ **CORRECTED — §15 governs (this section is superseded by it, see the banner above).** `llama-guard-3-8b` has native Hindi, so **en/hi are authoritative**; every *other* language is advisory→community-only and can never drive an autonomous adverse verdict. Independently confirmed 2026-07-25: Cloudflare lists Hindi among the model's supported languages. Two caveats stay live: AI Gateway **Guardrails** hi coverage is a switch-on verification item (§18.4), and the Tier-0 lexical floor does not depend on it either way.

**Anti-incitement / anti-doxxing tripwire:** on a violence-intent or **private-individual PII** hit → immediate quarantine → brief human-confirm hold → **short-purge the payload** (retaining violence-intent + PII is UAPA/BNS-radioactive), keeping only a minimal non-content audit row (the canonical `mod_audit` column set is defined once in §4.2 — this line and the §15 purge table previously gave two different sets). Radioactive payloads **cannot be retained-during-appeal** (gone by design) — appeals run off the audit record. The tripwire targets **private-individual** data only; **official-capacity accountability naming is out of scope** and routes to Domain-10's separate m-of-n gate, so the classifier cannot be weaponized to suppress legitimate misconduct records. Coordinated mass-flagging is itself a CIB input → trips into human review, not auto-suppression.
...
1. **Register metadata + hashes (tiny, first, always).** One sealed POST to `api` carrying the incident record **plus `original_sha256` and `derivative_sha256`** (typically <4 KB). The body is a framed sealed envelope — `MAGIC("HBE1",4B) ‖ algId(1B) ‖ seal()output` (`packages/worker-lib/envelope.ts`) — so the `api` Worker, holding no key, structurally rejects any non-sealed body (`application/octet-stream`, size-capped, magic-checked) before touching a binding; a plain-JSON body is refused by construction. Minimizing round trips is the whole game on 2G, so this one trip does three jobs: (i) records the incident as *documented* and returns an opaque receipt; (ii) runs the cohort-gated **exists-by-hash** verdict for the derivative → `derivative_held: skip|upload`; (iii) authorizes the vault upload and returns the opaque vault key (§4.4 ULID) + a `media` upload ticket. Seconds even on 2G — the "Saved. / Documented." moment. **Always** sends (tiny, safety-critical minimum), regardless of Data-Saver/battery.
...
**Media presign auth, as built (M3, 2026-07-25).** §7.6 above describes register returning a "media upload ticket" that the `media` Worker checks. That is **not** what ships, and the substitution is deliberate: a ticket needs a new shared secret between two Workers plus new state to track, and until it existed `/media/create` minted vault multipart uploads behind nothing but an IP-hash and ASN bucket. Every `/media/*` route now requires the **cap-cert + proof of possession** the codebase already has (compartment `document`), which binds the request to one key, one compartment and this exact body, once, and gives the per-credential rung of the rate ladder something to charge — no new secret, no new state, and strictly stronger than the sketch. Turnstile is still not re-checked there: its token is single-use and is spent at `/api/incidents/register`, which gates the whole flow. `partNumber` is bounded to R2's 1–10,000 at presign time, not only at complete time.
...
| Detainee real identity, family, Art.22/DK-Basu detail | **Off-platform only**; platform holds `{ref_hash, kind, next_deadline_hour, fired}` — an HOUR not a timestamp, and no case number or jurisdiction (§10.1) |
...
Content-free per-matter **Deadline-timer DO alarm** **marks a ref due and fires nothing outbound** (corrected at build, M5, 2026-07-26: its only effect is `fired = 1`, because a server that can reach the lawyer is a server holding a way to reach the lawyer, which is the compellable roster this platform refuses to build); the lawyer's device polls by ref token **over onion**, decorrelated, so the server sees only opaque-token polls, never the lawyer's full portfolio at once, never a source IP. **Incommunicado alert** fires only on **two independent authenticated triggers + human broker confirm**, rate-limited, content-free payload — abuse-resistant against agent-provocateur false alarms. Counsel defines the opaque-ref scheme, the incommunicado trigger, and whether any detainee field may transit the platform even transiently (hard gate).
...
| 7 | Detainee data & incommunicado | Platform holds `{opaque_ref, kind, next_deadline_hour, fired}` only — **an HOUR, never a timestamp** (corrected at build, M5, 2026-07-26: an Article 22 production deadline stored to the second **is an arrest timestamp**, since subtracting 24 hours yields the minute a named person was taken, and PITR keeps it ~30 days regardless); real identity off-platform; content-free DO-alarm poll over onion; two-trigger + human-broker incommunicado | **YES — hard gate; opaque-ref scheme + trigger definition** |
...
> **Updated (Session 2.7):** platform identity is a **comprehensive protest platform** — six core pillars, integrated into this milestone table, not a side track (PRD Session 2.7): (1) Get help / Give help; (2) **Document an incident** (a Home primary action; capture / upload / source-import, §7); (3) **public incident browse & search** — the theprotestrecord.com feature set built private (client-side search over a Cron-materialized **public-plaintext** `incident_public_index`, no query logging; only Verified/Community-Corroborated surfaced) — **read model M1, full pipeline + map M3**, behind a fail-closed `incidents_publish` flag; this was a gap and is now core; (4) **community as the day-1 second line** (verify/corroborate/comment/report as reversible state-machine inputs, §15; report-a-problem M1, corroborate + state machine M2) with **moderators as the third line** (Layer B, off the public app); (5) **accountability & legal tracking** (institutional dashboard + FIR/NHRC/court status, §8; aggregate read M5, individual naming m-of-n-gated OFF); (6) live board + detention KYR (§8.3). No item here loosens a red line, a kill switch, or a counsel gate.
...
| **M2 — Identity + moderation floor** | Domain 3 identity core (HKDF tree, stateless cap certs, Turnstile-with-refuse-not-plaintext-fallback); Tier-0 detectors + community flag as reversible state-machine inputs; moderator console behind Access. **Reach machinery is M3, not M2** (§18.1 governs): no reputation-gated reach, no √-damping, no CIB, no cohort-pivot here | Crypto audit passed; #5 verifier scheme signed off. **Human org staffing gates only the irreversible m-of-n unlocks (#6–#9), which ship OFF** — §15/§18.1 make the human org a hardening milestone, not a launch prerequisite for the reversible surface |
...
- **Access Independent MFA (GA 2026-04-15; re-verified 2026-07-25)** — Enforce a second factor in Access, decoupled from the IdP. **Correction RETRACTED (re-verified against the live docs 2026-07-25).** The previous text here claimed security-key-*only* is unenforceable and that TOTP cannot be excluded. **Neither claim is supported by any Cloudflare documentation.** The only documented single-method caveat is `piv_key`-specific: *"If PIV key is the only allowed MFA method ... users who access non-infrastructure applications will not see any available MFA method."* And Independent MFA is configurable at **three** levels — organization, **application, and policy**, with lower levels overriding higher — so "not a per-application field" was also wrong. The GA changelog says outright that you can *"enforce stricter authentication methods like hardware security keys on sensitive applications."* AAGUID pinning does still need an AAGUID List of real hardware. `piv_key` is infrastructure-apps only. *Change:* the shipped posture stays `allowed_authenticators = ["security_key","biometrics"]` (both WebAuthn, phishing-resistant) + `required_aaguids` restricted to ceremony hardware + AMR-matching **off** — AMR matching is worth keeping off on its own merits, since most IdPs omit AAGUID from the `amr` claim, so enabling it would let Access skip the prompt and quietly defeat AAGUID pinning. **Open item, deliberately not changed from a doc reading alone:** security-key-only now appears enforceable, at the application level if not the org level. Verify it empirically before relying on it, and treat it as a lockout risk — this is the only privileged surface, and a bad MFA config locks the operator out of the kill switches. Until tested, keep the WebAuthn-only pair and describe it as "WebAuthn-only (hardware key or platform biometric)", **without** the "TOTP not excludable" claim, which was never true.
...
| **Community-Corroborated** | **≤ 1.5 (autonomous ceiling)** | public label is "Confirmed by people nearby"; the "not yet human-verified" wording is tap-through only (§18.1 — **no "AI-checked" string ever reaches a user**). Never full amplification |
...
```
Clusters are **ephemeral, behavior-only** (ASN bucket, timing burst, device-class, stylometry-lite, template similarity), scoped to the item's DO window and **expired lazily on a tick grid** — **corrected at build (M5, 2026-07-26):** `CoordinationWindow` is in `gate-memory-only`'s `WHOLLY_MEMORY` set, and a wholly-memory class cannot arm an alarm, because `setAlarm()` reaches the same durable interface the gate forbids, bills as a row written, and leaves a PITR-visible record that something is pending at time T. Clusters past the window are therefore dropped when the object is next touched, and the whole object is lost on eviction. Eviction ending a window early only ever *reduces* clustering, which is the fail-safe direction: §15 gives synthesized disagreement a contested label and **no reach penalty**, so losing a cluster withholds a penalty rather than applying a wrong one — **no co-witness or social graph is ever persisted.** A new report increments `independent_source_count` **only** if it clears independence vs existing members: **distinct media provenance** (pHash recycled-media match ⇒ **1** source via `canonical_content_id` — kills "everyone reposts the same clip"), **text novelty**, **behavioral independence** (CIB-clear), **account independence**.
...
|---|---|
| Private-individual PII **authored as an attack** (doxx), or explicit violence-intent, `llama-guard ≥ 0.95` **AND** Tier-0 corroboration | **Short-purge the public payload** (retaining it is UAPA/BNS-radioactive), keep the non-content `mod_audit` row (column set in §4.2), **only if a recoverable vault original exists**; else **hide-not-purge**. **Note the tension, resolved in §4.2:** the fixed action enum contains no purge verb, so no autonomous purge path is built — radioactive handling is hide-pending + retain-pending + route-to-gate until a human acts |
...
**These are INTERNAL state descriptions, never public badge text.** §18.1 is authoritative: the public surface carries only the four plain words from PRD §15, and **no "AI-checked" string ever reaches a user** (`gate-ai-tells` enforces this on every shipped catalog). The honest "automated, can be wrong" wording lives on tap-through only.

| Internal state | Public label (the only thing a reader sees) | Tap-through meaning |
|---|---|---|
| Unverified | Not checked yet | Not checked or corroborated. Do not rely on this. |
| AI-Screened | Not checked yet | Automated checks found nothing unsafe. That is not the same as true. |
| Corroborating | Not checked yet | A couple of people said the same thing. Below the corroborated bar. |
| Community-Corroborated | Confirmed by people nearby | Local users marked this as real. Not team-confirmed. Use your judgement. |
| Disputed | Reported as a problem | Corroboration and refutation conflict. |
| Debunked | Reported as a problem | See attached correction. |
| Quarantine-Pending | *(hidden — no label renders)* | Held pending review. Retained, never deleted. |
| Human-Verified (Layer B) | Verified by our team | Someone on our team confirmed this and its contact. |

`Corroborating` maps to "Not checked yet" deliberately: it clears two corroborators and a 15-minute dwell, versus K=4 across three independence buckets plus AI concurrence for `Community-Corroborated`. Presenting it as confirmed would over-claim by design. The mapping is implemented in `apps/web/src/lib/verification-map.ts` and conformance-tested.
...
- **Images:** `env.IMAGES` binding → long-edge ≤1600–2048px (20 MB input ceiling; oversized → **skip, never fail**). ~4 MB JPEG → ~180–300 KB.
- **CORRECTED 2026-07-25 — the master ships as WebP, not AVIF.** The live Images limits table lists *"Image dimension, AVIF | 1,200 pixels"* against **12,000** for every other format, and does not say whether that bounds input or output. AVIF *input* is Enterprise-only, which makes an input reading likely but not certain. §7.5's **1,280 px legibility floor** is what keeps a badge number readable and already outranks byte targets, so a format that might silently cap below it cannot be the default: the failure would be a public copy nobody can read, discovered after the fact. `MASTER_FORMAT` in `workers/media/src/app.ts` is one constant away from AVIF once someone measures a 2048 px AVIF output against a real account. **Also verified 2026-07-25:** the binding is configured as a top-level `{"images": {"binding": "IMAGES"}}` with **no resource id**, so it needs no `infra/outputs.tf` output and no `REPLACE_*` injection; `.info()` is free and returns no pixels (confirming a server-side pHash is impossible through it); billing is per unique transformation once per calendar month; and responses are **not cached automatically**, so `"cache": {"enabled": true}` is set alongside it.
...
### 17.1 Tooling & state
- **OpenTofu** (`tofu`, **≥1.12**; the pin is conservative and correct. **Corrected 2026-07-25:** the "1.11.x is EOL 2026-08-01" claim was wrong — OpenTofu supports the latest three minors, so 1.12/1.11/1.10 are all current and 1.11 EOLs only when 1.13 ships, date unannounced) + **Cloudflare provider `~> 5.22`**. State in **R2** via the Terraform `s3` backend with `use_lockfile = true` (native state locking, no DynamoDB). `backend.hcl` is gitignored and rendered inline in CI; state-bucket auth is **R2 bucket-scoped keys** passed as `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` — separate from the Cloudflare API token.
...
- **`ci.yml`** (PR): install `--frozen-lockfile` → typecheck → build → **CI-as-enforcement gates** (§17.5) → `tofu fmt -check` + `tofu validate`. **Changed (Session 3):** PRs run *validate*, not *plan* — on a public repo, `tofu plan` against PR-modified config is arbitrary code execution holding the Terraform token (a malicious provider/data-source exfiltrates it), so no credential enters any PR job. The plan is produced and read in the approved `infra` job (below), and the maintainer can run `tofu plan` locally read-only via the bootstrap-rendered backend config.
...
- **Domain-separation gate** — `gate-sig-context` fails the build if a curve's `.sign()` is reached directly outside `packages/crypto/src/`, or if the `SIG_CONTEXT` registry loses its `as const` (which would widen the literal union to `string` and stop TypeScript rejecting ad-hoc tags) or gains a duplicate, unprefixed or unversioned tag. Two protocols sharing a signing tag typechecks perfectly and defeats the framing entirely, so it needs a gate rather than a review note.
...
- **PoP anti-replay:** per-request proof-of-possession binds a short-window timestamp + nonce; the api Worker rejects stale/seen nonces (bounded in the RateLimit memory DO).
- **Cap-cert + PoP, as built (M2).** The certificate is **self-issued**: no issuer, no issuance endpoint, no platform-held key, which is what keeps "no session table, no account row" literally true. It therefore provides **zero Sybil resistance** — anyone can mint unlimited certificates for free — and it must never be read as an authorization grant. Resistance is Turnstile + the rate ladder + reputation. What it buys is a stable pseudonymous per-compartment subject to rate-limit and accrue reputation against, with no roster on the platform.
  - Layout `"HBC1" ‖ version ‖ algId ‖ compartment ‖ issuedAt ‖ expiresAt ‖ pubkey ‖ selfSig`; the PoP signs `certHash ‖ len-prefixed method ‖ len-prefixed path ‖ timestamp ‖ nonce ‖ bodyHash`. Method and path are length-prefixed so `("POST","/a/b")` and `("POSTX","a/b")` cannot serialise alike, and the **body hash** is what stops a captured credential being reused to post different content.
  - **One compartment per request, enforced:** Workers join repeated headers with `", "`, so two credentials arrive comma-joined and are refused outright rather than one being picked.
  - **Ladder order** (`packages/worker-lib/src/ratelimit.ts`): 16 fixed global shards and one bucket per ASN run first and together, then signature verification, then the per-credential bucket which also holds that credential's nonces. Throttling before the per-credential rung is what stops an attacker minting unbounded DO instances with free certificates; co-locating the nonce with its credential means a replay necessarily lands on the instance that remembers it, at no extra round trip.
  - **Honest limit:** a memory-only DO forgets its nonces on eviction, so a proof captured moments before eviction could be replayed inside its short freshness window. Durable nonces would create exactly the compellable per-request record the charter forbids, and would bind a nonce to a credential in storage. Eviction is the correct trade; exposure is bounded by the PoP freshness policy, not by that memory.
  - Every rejection is a flat 401 with no detail — naming the failing check would hand an attacker an oracle for probing clock skew and nonce state.
...
| `CoordinationWindow` | **memory-only** (CIB; lazy tick-grid expiry, never an alarm — see §15) | wholly-memory | `api` | M3 |
...
**KV:** `FLAGS` (**M0**, fail-closed reads, 60 s TTL cache of FlagState; ~120 s worst-case flip latency), `CONFIG` (**M0**), `I18N` (M1), `KEYDIR_CACHE` (M1), `RULESETS` (M2).
**Queues** (provisioned M0): `moderation-bulk` + DLQ (consumer M2); `life-safety` + DLQ (consumer M4; separate queue + operator paging on `metrics().oldestMessageTimestamp` — there is no "reserved concurrency" primitive, see §3.1).
...
**Access:** one application for the console hostname, `prevent_destroy`. **M0.** MFA is **WebAuthn-only (hardware key or platform biometric)** via org-level Independent MFA `allowed_authenticators = ["security_key","biometrics"]` + `required_aaguids` (ceremony hardware) + AMR-matching off — a hardware-dependent RUNBOOK step (see the §14 correction: per-application and per-policy MFA config **do** exist, and the "security-key-only is not enforceable" claim was retracted as unsupported).

### 18.4 Freshness updates (re-verified today; §14 remains otherwise accurate)

- **OpenTofu ≥ 1.12.** §17.1. (The earlier "1.11.x EOL 2026-08-01" note was incorrect — see §17.1.)
...
- **New Cloudflare capabilities to adopt** (all confirmed GA, currently under-used): **Access Independent MFA** (org-level `allowed_authenticators = ["security_key","biometrics"]` **AAGUID-restricted** to ceremony hardware, AMR-matching off — security-key-*only* is not enforceable, §14 correction; RUNBOOK manual step); **DO Memory-Usage metrics** — alarm on `memoryUsageBytesP99` per memory-only DO + a byte-threshold shard-out trigger (memory-only DOs can hit silent `Exceeded Memory` at surge) (§3.2, §10.5); **Queues `metrics()`** — the life-safety queue pages the operator when `oldestMessageTimestamp` exceeds N seconds (§3.1); **Container active-CPU-second billing + custom instance types** — transcode/pHash is materially cheaper than the prior estimate; size a custom instance rather than over-provisioning (§16).
- **Precision notes:** AI Gateway Guardrails — soften "GA; Hindi supported" to "available; confirm GA + hi coverage at switch-on; the Tier-0 floor does not depend on it." Queues **5,000 msg/s is a plain per-queue throughput limit** (re-verified 2026-07-25 against the current limits page), not a pull-consumer-only ceiling; size the protest-day drain off both it and the 250 push-concurrency figure. `llama-guard-3-8b` is still current (no Llama-Guard-4). Keep Capacitor 8 (9 is alpha).
...
### Build notes from implementing this section (M3, 2026-07-25)

- **One render, not two.** `renderDerivative` (in `apps/web/src/lib/pipeline/pipeline.worker.ts`) returns the *encoded* bytes; the confirm screen displays those bytes; those same bytes commit. There is no second render path between approval and storage, so there is nothing to drift. `LocalDocument.confirmedDerivativeSha` records which bytes were approved and `save()` refuses a mismatch — a regression guard, not a second source of truth.
- **Deliberate deviation from the literal step order.** The crash-quarantine copy (step 1) and the seal (step 9) are the SAME sealing pass. Sealing twice would run XChaCha20 over a multi-MB file twice on the weakest device we support. The original is sealed once, up front, and that ciphertext is both the crash copy and the vault artifact — which satisfies the "always hashed and always sealed" invariant earlier than the written order, not later.
- **The coverage assertion insets by a FIXED 3 px, never a fraction of the box.** An 8%-of-box inset passed every test and was badly wrong: on a 700 px cover box it skips a 56 px band *inside* the region the human confirmed, so a paint offset of tens of pixels along an edge leaves part of a face uncovered and still verifies clean. Ringing is a property of the codec, not of the box. Found by deliberately sabotaging the paint offset by 6 px and watching nothing fail; the sampling also now always includes the last index on each axis, since a plain strided loop stops short of the far edge and left the bottom and right sides of a large box unchecked.
- **`connect-src` is not widened for `blob:`.** The pixel test cannot `fetch()` the object URL the confirm screen displays, because the app's CSP forbids it — correctly. The test reads the committed bytes out of IndexedDB instead, which also makes it an assertion about the artifact that was *stored* rather than the one that was *rendered*. Widening `connect-src` for test convenience would weaken the directive an XSS on a browser-crypto PWA exfiltrates over.
- **Dimensions come from the file header** (`image-header.ts`: PNG/JPEG/WebP/GIF/BMP), then `ImageDecoder`, and only then a full decode. The M1 code called `createImageBitmap(blob)` with no resize options, which is an unbounded decode — a 108 MP capture is ~432 MB of RGBA on a 1–2 GB phone.

**Outbox runner (M3). Seven corrections to this section, made while building `apps/web/src/lib/outbox-runner.ts`.**

- **Concurrency is across DOCUMENTS, not across parts.** The "1–2 on 3g, 2–4 on 4g" line above sits under the multipart heading and literally means parts. Implementing it that way requires redefining `MultipartCursor.nextPart` as "highest handed out" plus an in-flight reconciliation on resume — a semantic migration of the one persisted structure that anchors chain of custody, and it breaks the ETag-persisted-before-nextPart-advances invariant. On 2G, the tier this section is most emphatic about, the answer is 1 either way, and the common queue is several stills rather than one huge video. Bounding concurrent documents lands inside the stated envelope at every tier for none of that risk.
- **Two `LinkClass` types are deliberate.** `derivative-core`'s is two-tier (`slow`/`fast`) and governs encode quality; the outbox's is three-tier and governs upload concurrency. The two-tier type is *assignable* to the three-tier one, so a stray reuse typechecks silently and would put three concurrent uploads on 3G. `uploadLinkFrom` is the only bridge, and it has its own test.
- **Nothing is flushed autonomously before a receipt exists.** `POST /api/incidents/register` requires a live single-use Turnstile token, so phase 1 cannot resume in the background. Persisting the token would leave a personhood token on a seized phone; flushing with an empty one is a guaranteed 403 per attempt, spending 2G data while the UI claims to be sending. Refusing matches the earlier rule that phase 1 is not a silent default for a high-risk capture.
- **`maxAge` bounds RETRYING, not retention.** Past it the runner stops picking an item up on its own; the row and the ciphertext stay and no status changes. `original_status = lost` is set only by a *detection* that the backing bytes are gone (eviction, a deleted document, an erase) — never by a clock. Auto-destroying evidence on a timer would be the most dangerous behaviour this feature could have.
- **Cancel stops sending and does not destroy the document.** The instruction to "wipe the cipher blob" assumes the data model where the ciphertext lives inside the queue row. In this codebase it lives on `LocalDocument.original.sealed`, shared with the kept-on-phone document, so wiping it on cancel would silently destroy a pristine original because someone tapped "stop sending".
- **The device erase does NOT remove the account by default.** The rule above says the wipe clears IndexedDB, which would include `harborage-identity`. It is now an unchecked box on the confirm step. The two wipes protect different people: the document wipe protects the people *in* the photographs and must work in one tap at a checkpoint, while the identity wipe protects the user's own pseudonymity and is unrecoverable because backup words are erased by default. The safer default is the narrower destruction.
- **The erase makes no network call.** No `AbortMultipartUpload` per pending item: an erase must be fast, must work offline, and must not emit a legible "this person just wiped their phone" burst at the moment someone is under duress. The `abort-incomplete-multipart-30d` lifecycle rule on the evidence-vault bucket collects the strays.
- **Two bugs found only by sabotage, both worth remembering.** (a) The concurrency cap is computed per flush, so a foreground flush racing a manual "Try now" ran *two* uploads on 2G — guarding the event handler was not enough, because the manual path calls the flush directly; flushes are serialised globally now. (b) `deleteDatabase` blocks silently while any connection is open and the runner holds a long-lived one, so the erase reported success while leaving `harborage-outbox` in place; and `openDB` *creates* a database, so the page load after an erase recreated what had just been removed. Readers check existence before opening; only a writer creates.
