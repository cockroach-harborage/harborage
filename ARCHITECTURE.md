...
- `reputation_scalars` — `{pubkey_hash, reputation_scalar, coarse_counters, epoch}`, per-compartment, no edges. **Milestone corrected: M3, not M2** — §18.1 puts the whole reputation machinery at M3, and creating the table early would accumulate a compellable per-compartment roster over the D1 Time-Travel window while gating nothing (its stated mitigation, blind-token-carried reputation, has no audited implementation to build on yet).
