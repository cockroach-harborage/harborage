...
- **Access Independent MFA (GA 2026-04-15; re-verified 2026-07-25)** — Enforce a second factor in Access, decoupled from the IdP. **Correction:** you **cannot** enforce security-key-*only* — if one method is the sole allowed one, users see no available MFA method, and TOTP cannot be excluded at the org level. Independent MFA config (`allowed_authenticators`, `required_aaguids`, `session_duration`, AMR-matching) is an **organization-level** setting (`PUT /accounts/{id}/access/organizations`, `mfa_config`), not a per-application field, and AAGUID pinning needs an AAGUID List of real hardware. `piv_key` is infrastructure-apps only. *Change:* strongest achievable posture on the console is org-level `allowed_authenticators = ["security_key","biometrics"]` (both WebAuthn, phishing-resistant) + `required_aaguids` restricted to ceremony hardware + AMR-matching **off**; set as a RUNBOOK manual step (org-scoped + hardware-dependent), and correct any "hardware FIDO2 only" wording to "WebAuthn-only (hardware key or platform biometric); TOTP not excludable."
...
**Access:** one application for the console hostname, `prevent_destroy`. **M0.** MFA is **WebAuthn-only (hardware key or platform biometric)** via org-level Independent MFA `allowed_authenticators = ["security_key","biometrics"]` + `required_aaguids` (ceremony hardware) + AMR-matching off — an **org-scoped, hardware-dependent RUNBOOK step**, not per-app tofu (see §14 correction; security-key-*only* is not enforceable).
